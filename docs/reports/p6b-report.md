...
**1 · Scroll choreography.** Each of the 8 non-hero sections gets one gentle fade/rise on first entry — a single reveal per section, never a per-element cascade. New `initSectionReveal` IIFE in `site/js/site.js` uses a **one-shot** IntersectionObserver (safe here — it fires once and is coarse, immune to the two IO pitfalls that forced the nav patterns off IO). Guards: reduced-motion bails to the static end-state (CSS `.reveal` override + JS `matchMedia` bail); a geometry check on load reveals anything already in view so a section can never stick hidden. Transform/opacity only, site easing. Hero excluded (keeps its own `.loaded` entrance).
